...
```{r}
Ktunnused <- flight[, sapply(flight, is.factor)]

# Kui Ktunnused on vektor, teisenda see andmetabeliks
if (is.vector(Ktunnused)) {
  Ktunnused <- as.data.frame(Ktunnused)
  colnames(Ktunnused) <- names(flight)[sapply(flight, is.factor)]
}

# Joonista histogrammid
par(mfrow = c(1, 2))
par(mar = c(10, 4, 4, 2))
